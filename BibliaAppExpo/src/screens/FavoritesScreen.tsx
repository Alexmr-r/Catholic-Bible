...
// Listas de libros para filtrado client-side
  ...
  const loadFavorites = async (showLoading = true) => {
    try {
      ...
      if (cachedData && cachedData.length > 0) {
        const transformed = transformFavorites(cachedData);
      ...
      if (isOnline) {
        try {
          ...
          const transformed = transformFavorites(response.favorites);
          setFavorites(transformed);
        } catch (apiErr) {
          // Si ya tenemos datos de caché, no mostrar error
          console.warn('[FavoritesScreen] Error API, usando caché si hay:', apiErr);
        }
      } else {
  ...
  const transformFavorites = (apiFavorites: ApiFavorite[]): Favorite[] => {
    return apiFavorites.map((fav: ApiFavorite) => {
      ...
      return {
        ...
        verseEnd: isFullChapter ? 999 : verseEnd,
      };
    });
  };
      ...
      <View style={styles.container}>
        ...
        {isLoading ? (
          ...
          <FlatList 
            ...
            refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={onRefresh} tintColor={colors.primary.DEFAULT} />} 
            ListEmptyComponent={
